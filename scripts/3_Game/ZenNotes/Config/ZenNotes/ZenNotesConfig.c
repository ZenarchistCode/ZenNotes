ref ZenNotesConfig g_ZenNotesConfig;

static ZenNotesConfig GetZenNotesConfig()
{
	if (!g_ZenNotesConfig) GetZenConfigRegister().RegisterConfig(ZenNotesConfig);
	return g_ZenNotesConfig;
}
...
modded class ZenConfigRegister
{
	override void RegisterPreload()
	{
		...
		RegisterType(ZenNotesConfig);
	}
}

class ZenNotesConfig: ZenConfigBase
{
	// -------------------------
	// CONFIG SETTINGS
	// -------------------------
	...
	{
		return JsonFileLoader<ZenNotesConfig>.LoadFile(path, this, err);
	}

	override bool WriteJson(string path, out string err)
	{
		return JsonFileLoader<ZenNotesConfig>.SaveFile(path, this, err);
	}
	...
	bool IsBlacklisted(string msg)
	{
		...
		{
			checkMsg.Replace(STRIP_CHARS.Get(i), "");
		}
